...
export async function GET(request: NextRequest) {
  try {
    ...
    const response = await backendApiClient.get(API_ENDPOINTS.SLA_TEMPLATES.LIST, {
      params,
      headers: {
    ...
    const err = error as
      ...
      | undefined
    console.error('API Route /api/sla-templates error:', error)
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const response = await backendApiClient.post(API_ENDPOINTS.SLA_TEMPLATES.CREATE, reqBody, {
    ...
    const err = error as
      ...
      | undefined
    console.error('API Route /api/sla-templates POST error:', err?.response?.status || err?.message)
    ...
    if (err?.response?.status === 401) {
      try {
        ...
        const retryResp = await backendApiClient.post(
          API_ENDPOINTS.SLA_TEMPLATES.CREATE,
          originalBody,
          {
