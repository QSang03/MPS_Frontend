...
export async function GET(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const response = await backendApiClient.get(API_ENDPOINTS.SERVICE_REQUESTS.MESSAGES(id), {
...
export async function POST(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    const response = await backendApiClient.post(
      API_ENDPOINTS.SERVICE_REQUESTS.MESSAGES(id),
      reqBody,
      {
        headers: { Authorization: `Bearer ${accessToken}` },
      }
    ...
    if (err?.response?.status === 401) {
      try {
        ...
        const retryResp = await backendApiClient.post(
          API_ENDPOINTS.SERVICE_REQUESTS.MESSAGES(retryId),
          originalBody,
          {
