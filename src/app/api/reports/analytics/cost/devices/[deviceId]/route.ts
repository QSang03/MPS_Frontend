...
export async function GET(request: NextRequest, ctx: unknown) {
  ...
  try {
    ...
    console.log('[API] Device cost request:', { deviceId, params: paramsFromQuery })
    ...
    const response = await backendApiClient.get(`/reports/analytics/cost/devices/${deviceId}`, {
    ...
    console.log('[API] Device cost response:', response.data)
    ...
    console.error('[API] Device cost error:', {
      message,
      status,
      data,
    ...
    return NextResponse.json(
      {
        success: false,
        message: message || 'Failed to fetch device cost',
