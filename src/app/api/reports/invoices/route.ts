...
export async function GET(request: NextRequest) {
  try {
    ...
    const response = await backendApiClient.get(API_ENDPOINTS.REPORTS.INVOICES.LIST, {
      params,
      headers: {
    ...
    const err = error as
      ...
      | undefined
    console.error('API Route /api/reports/invoices GET error:', error)
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const response = await backendApiClient.post(API_ENDPOINTS.REPORTS.INVOICES.CREATE, reqBody, {
    ...
    console.error('API Route /api/reports/invoices POST error:', error)
    ...
    if (err?.response?.status === 401) {
      try {
        ...
        const retryResp = await backendApiClient.post(
          API_ENDPOINTS.REPORTS.INVOICES.CREATE,
          originalBody,
          {
