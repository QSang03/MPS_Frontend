...
export async function GET(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    if (sp.has('consumableTypeId')) query.consumableTypeId = sp.get('consumableTypeId')

    const response = await backendApiClient.get(`${API_ENDPOINTS.CUSTOMERS}/${id}/consumables`, {
      headers: {
    ...
    const err = error as
      ...
      | undefined
    console.error('API Route /api/customers/[id]/consumables GET error:', error)
