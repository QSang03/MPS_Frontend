  ...
  const loadConsumables = useCallback(
    async (p = 1) => {
      try {
        ...
        // backend will scope by session customer for user
  ...
  return (
    <>
      <Card>
        <CardHeader>
          <div className="flex items-center justify-between">
            <div>
              ...
              <CardDescription>Vật tư liên quan đến khách hàng của bạn</CardDescription>
            </div>
                ...
                <table className="w-full">
                  <thead className="bg-gradient-to-r from-emerald-50 to-teal-50">
                    <tr>
                      ...
                      <th className="px-4 py-3 text-left text-sm font-semibold">Đã lắp</th>
                    ...
                    {filteredConsumables.map((c: Record<string, unknown>, idx: number) => (
                      ...
                      >
                        ...
                        <td className="px-4 py-3 text-sm">
                          {String((c.consumableType as Record<string, unknown>)?.partNumber ?? '-')}
                        </td>
                        ...
                        <td className="px-4 py-3 text-sm">
                          {Number(c.deviceCount as unknown as number) > 0 ? (
                            <span className="font-medium text-emerald-600">Đã lắp</span>
                          ) : (
        ...
        {/* Create button for users who can create consumables */}
        <div className="p-4">
          <ActionGuard pageId="consumables" actionId="create">
            {/* reuse system BulkAssignModal for batch create */}
            <BulkAssignModal
              trigger={<Button className="bg-emerald-600 text-white">Tạo Vật tư</Button>}
            />
          </ActionGuard>
        </div>
      </Card>
